{pkgs, ...}: {
  # Включаем поддержку KVM
  boot.kernelModules = ["kvm-intel"];

  # Основные сервисы виртуализации
  virtualisation = {
    libvirtd = {
      enable = true;
      qemu = {
        ...
        ovmf = {
          enable = true;
          packages = [pkgs.OVMFFull.fd]; # Для UEFI загрузки
        };
        swtpm.enable = true; # Поддержка TPM для Windows 11
      };
      onBoot = "ignore"; # Не стартовать VM автоматически
      onShutdown = "shutdown"; # Корректно выключать VM при остановке системы
    };
    spiceUSBRedirection.enable = true; # Для USB
  };

  # Включаем дефолтную сеть
  ...
  # Создаём Windows 11 VM
  systemd.services.libvirt-win11 = {
    description = "Windows 11 VM configuration";
    requires = ["libvirtd.service" "libvirtd-default-network.service"];
    after = ["libvirtd.service" "libvirtd-default-network.service"];
    wantedBy = ["multi-user.target"];
    path = [pkgs.libvirt];
    script = ''
  ...
  environment.systemPackages = with pkgs; [
    ...
    virt-viewer # Просмотрщик VM
    spice
    spice-gtk # Для улучшенной графики
    win-virtio # Драйверы для Windows
    swtpm # TPM эмулятор
  ];
